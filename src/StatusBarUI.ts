...
import { formatTime, generateProgressBar } from "./utils";
...
export class StatusBarUI {
  private leftBorder: vscode.StatusBarItem;
  private progressItem: vscode.StatusBarItem;
  private dotItem: vscode.StatusBarItem;
  private timerItem: vscode.StatusBarItem;
  private buttonItem: vscode.StatusBarItem; // Play/Pause
  private stopItem: vscode.StatusBarItem; // Stop (only when active)
  private rightBorder: vscode.StatusBarItem;

  constructor() {
    // Right alignment - higher priority = further right
    // Create items in order from left to right
    this.leftBorder = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
      STATUS_BAR_PRIORITY + 6,
    );
    ...
    this.dotItem = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
      STATUS_BAR_PRIORITY + 4,
    );

    this.timerItem = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
      STATUS_BAR_PRIORITY + 3,
    );

    this.buttonItem = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
    ...
    this.rightBorder = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
      STATUS_BAR_PRIORITY,
    );
  ...
  public update(
    ...
    this.leftBorder.text = SEPARATOR_LEFT;
    ...
    this.rightBorder.text = SEPARATOR_RIGHT;
  ...
  public showIdle(): void {
    const emptyBar = EMPTY_BLOCK.repeat(PROGRESS_BAR_LENGTH);
   ...
   * Get tooltip text
   */
  private getTooltip(state: TimerState, sessionType: SessionType): string {
   ...
   */
  private getSessionLabel(sessionType: SessionType): string {
    switch (sessionType) {
      case "work":
        return "Work session";
      case "shortBreak":
   ...
   */
  private applyTheme(state: TimerState, sessionType: SessionType): void {
    ...
    if (state === "running") {
      if (sessionType === "work") {
        ...
        this.timerItem.backgroundColor = new vscode.ThemeColor(
